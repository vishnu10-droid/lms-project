  ...
  if (!activeChat) {
    return (
      <div className="p-6 bg-gray-100 min-h-screen flex flex-col items-center">
        <h1 className="text-3xl font-bold mb-8 text-gray-800">My Messages</h1>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-5xl">
          {chatOptions.map((chat) => (
            <div 
              key={chat.id}
              onClick={() => setActiveChat(chat)}
              className="bg-white p-8 rounded-3xl shadow-md hover:shadow-xl transition-all cursor-pointer border-b-4 border-transparent hover:border-slate-400 relative group"
            >
              {chat.unread > 0 && (
              ...
              <h2 className="text-xl font-bold text-gray-800">{chat.title}</h2>
              <p className="text-gray-500 mt-2">Click karke chat shuru karein...</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col h-screen bg-white">
      ...
      <div className="flex items-center p-4 border-b bg-white sticky top-0 z-10 shadow-sm">
        ...
        <div>
          <h3 className="font-bold text-gray-800">{activeChat.title}</h3>
      ...
      <div className="flex-1 overflow-y-auto p-4 bg-slate-50 space-y-4">
        <div className="bg-white p-3 rounded-2xl rounded-tl-none shadow-sm max-w-[80%] self-start border border-gray-100">
          <p className="text-sm text-gray-800 font-medium mb-1 text-blue-600">System</p>
          <p className="text-gray-700">Welcome to {activeChat.title}! Aap yahan apne sawal pooch sakte hain.</p>
        </div>
